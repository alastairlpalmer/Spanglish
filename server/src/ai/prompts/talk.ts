...
export function talkSystemPrompt(
  req: Pick<TalkRequest, 'scenario' | 'dialect' | 'level' | 'weakConcepts' | 'targetWords'>,
): string {
  const weak = req.weakConcepts.length
    ...
    : '';
  const targets = req.targetWords.length
    ? `\nWeave these words the learner is currently studying into your replies naturally, and create openings that invite the learner to use them: ${req.targetWords.join(', ')}. NEVER announce that you are doing this.`
    : '';
